...
if not args.skip_test:
    ...
    if RAND_SEED == 0:
        ...
        for i in range(9):
            x = i % 3
            y = i // 3
            axarr[x, y].imshow(vector_to_raster(test_imgs[worst[i]],padding=0, line_diameter=16),cmap='gray_r',vmin=0,vmax=255)
            axarr[x, y].set_title(f"\"{list_of_classes[find_class(worst[i], test_counts)]}\"", pad=0)
            axarr[x, y].axis('off')
        plt.suptitle("QD-3: Augmentation-CNN")
